...
class RabbitMqHttpApiClient {
     ...
     * @throws \RuntimeException
     */
	public function declareExchange($vhost, $exchange, array $attributes = array()) {
	...
	public function listPoliciesOf($vhost, $name = NULL) {
		$path = null === $name
			? 'policies/' . urlencode($vhost)
			: 'policies/' . urlencode($vhost) . '/' . urlencode($name);
	...
	public function listParameters($component = NULL) {
		$path = null === $component
			? 'parameters'
			: 'parameters/' . urlencode($component);
	...
	public function listParametersOf($component, $vhost, $name = NULL) {
		$path = null === $name
			? 'parameters/' . urlencode($component) . '/' . urlencode($vhost)
			: 'parameters/' . urlencode($component) . '/' . urlencode($vhost) . '/' . urlencode($name);
     ...
     * @throws \RuntimeException
     */
	private function requestPut($path, array $requestVars = array()) {
		curl_setopt($this->curl, CURLOPT_CUSTOMREQUEST, 'PUT');
		$url = $this->buildGetUrl($requestVars, $this->getServiceUrl($path));
     ...
     * @throws \InvalidArgumentException
     */
	private function requestDelete($path, array $requestVars = array()) {
     ...
     * @throws \InvalidArgumentException
     */
	private function requestPost($path, array $requestVars = array()) {
     ...
     * @throws \InvalidArgumentException
     */
	private function requestGet($path, array $requestVars = array()) {
	 ...
	 * @return string
	 */
	private function buildPostBody(array $data = array()) {
